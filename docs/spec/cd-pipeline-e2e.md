...
1. **Uniform deploy path** — every node app uses image build → per-node deploy-state update → Argo sync. For an external-compute selection, deploy state contains a provider-neutral `ComputeWorkload`; its controller converges Akash without creating a second CI or REST deployment lane ([ci-cd.md](./ci-cd.md) Axioms 23 and 26).
2. **Declared networking** — k3s apps use Services; an external multi-service workload derives private service DNS bindings and one public hostname from the same provider-neutral declaration
