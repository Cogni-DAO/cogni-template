...
A value's origin is independent of whether it must be mirrored outward. The GitHub App **webhook secret** is `source: agent` (we generate it) **and** `syncTo: github-app-webhook` (GitHub holds a copy that must byte-match). Per-node LiteLLM virtual keys use the same two-axis contract: OpenBao owns the generated bytes and LiteLLM holds the authentication copy. `syncTo` names the external mirror; provisioning runs the matching push after the OpenBao write is durable.
...
| `litellm-virtual-key` | LiteLLM virtual-key registry             | `scripts/ci/secret-materialize.sh` (hash/alias lookup → explicit `POST /key/generate`), invoked by catalog-target materialization    |
