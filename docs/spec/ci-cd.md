...
- External app placement has one narrow substrate precondition in the normal flight/promotion workflows: when the typed planner selects any external target, an environment-serialized prerequisite renders the **complete** env allowlist from catalog `compute_egress_cidrs` and invokes the existing VM hardener through the environment's existing SSH identity. It accepts no caller CIDRs, converges additions and removals idempotently, and must succeed before a `ComputeWorkload` is written. K3s-only flights skip it. This reuses the infra/provision renderer and hardener without running broad `deploy-infra` or introducing a second firewall writer.
