...
- Every deployment-node flight/promotion has one narrow substrate precondition: an environment-serialized prerequisite renders the **complete** env allowlist from catalog `compute_egress_cidrs`, compares it with the installed full-file hash, and invokes the existing VM hardener through the environment's existing SSH identity only on drift. Running for k3s placements too is what closes the final Akash-removal hole; their steady state pays only the read/hash guard. It accepts no caller CIDRs, converges additions and removals idempotently, and must succeed before a `ComputeWorkload` or k3s app update is written. All existing workflow callers of the hardener share the `environment-substrate-<env>` concurrency domain, and staged files are run-unique. This reuses the infra/provision renderer and hardener without running broad `deploy-infra` or introducing a second firewall writer.
