...
- External app placement never turns app promotion into a host-firewall writer. The existing env infra/provision reconciliation renders `compute_egress_cidrs` from catalog membership and applies the resulting least-privilege substrate allowlist as a one-time platform prerequisite. Candidate/preview/production promotion then stays the same app-only GitOps operation.
