...
The publish gap is the same residual-trust the condition documents: it constrains SHAPE, not VALUES or REPLAY. Only the `EmissionsExecutor` closes it fully on-chain. Until then, mitigate with a Safe m-of-n executor.
...
> **Sunday-run operational HARD RULE (freeze-on-manifest-exists):** because the freeze keys on *"a manifest exists"* (not a recorded published-flag), a folded epoch is **final** — it can no longer be re-folded, even before publish. **Fix-ups (late wallet links, missed receipts, corrections) go to the NEXT epoch's cumulative fold; never re-finalize/re-fold a folded epoch to "patch" it.** This is safe because the model is cumulative (the next epoch's fold includes all credits to date). If a recovery path that re-folds an *unpublished* epoch is ever required, switch to a recorded `published_root` (Stage 1.5) so the freeze gates only on published — do NOT weaken the freeze.
