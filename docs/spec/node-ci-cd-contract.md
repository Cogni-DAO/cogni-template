...
**A divergent deployment parent is synchronized by patch, never by history replacement.** `TEST_PARENT_SYNC_IS_PATH_SCOPED`: the test parent is a long-lived fork with its own fleet/catalog identity. Cutover starts from test-parent `main` and applies the explicit file delta required by the reviewed control-plane commits onto a new test-parent branch. It does **not** merge or rebase canonical `main`, reset the fork, replace its catalog/roster, or import unrelated canonical history. Review the resulting PR against the test-parent base and preserve every unrelated test-only file. Only after that path-scoped PR is merged and its required contract/workflow/root/AppSet files are verified may the canonical root be switched to follow it.
