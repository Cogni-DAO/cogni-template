...
| Rule                             | Constraint                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| RECEIPT_ECONOMIC_CORE_IMMUTABLE  | `receipt_id`, source/event identity, stable actor ID, event time, economic context hash, and first-seen provenance never update. The DB trigger rejects DELETE and every UPDATE except `display_snapshot`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| RECEIPT_DISPLAY_REFRESHABLE      | `display_snapshot` is a versioned `ReceiptDisplaySnapshotV1` containing latest-known login, artifact URL, and presentation metadata. An equal-economic-core replay refreshes this JSONB atomically; it never changes `payload_hash` or another core column.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| RECEIPT_IDEMPOTENT               | `receipt_id` derives from immutable provider object identity (e.g., `github:pr:<providerRepoId>:42`), not mutable owner/repo text. Same-ID + equal economic core is a counted duplicate and display refresh; same-ID + different core is a permanent conflict that rolls back the batch. There is no silent PK skip.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| WRITES_VIA_TEMPORAL              | All write operations (collect, finalize) execute in Temporal workflows via the existing `scheduler-worker` service. Next.js routes return 202 + workflow ID. **Exception:** receipt ingest is exempt — webhook receivers may insert directly because equal-core retries only refresh display and divergent cores roll back.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| WEBHOOK_RECEIPT_APPEND_EXEMPT    | Webhook receipt insertion is exempt from WRITES_VIA_TEMPORAL per RECEIPT_IDEMPOTENT + RECEIPT_ECONOMIC_CORE_IMMUTABLE. Economic facts remain immutable; only the display snapshot refreshes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
**Postgres** stores immutable receipt economic cores beside refreshable display snapshots, with DB-trigger enforcement of that boundary.
...
4. **Inserts** with equal-or-conflict semantics: equal economic core refreshes `ReceiptDisplaySnapshotV1`; divergent core fails the batch permanently
...
### `ingestion_receipts` — immutable economic receipts + mutable display (Layer 1)

| Column             | Type        | Notes                                                                             |
| ------------------ | ----------- | --------------------------------------------------------------------------------- |
| `node_id`          | UUID        | NOT NULL — part of composite PK (NODE_SCOPED)                                     |
| `receipt_id`       | TEXT        | Canonical immutable event identity (e.g., `github:pr:<providerRepoId>:42`)        |
...
| `platform_login`   | TEXT        | First-seen display name retained in the immutable source record                   |
| `artifact_url`     | TEXT        | First-seen activity link retained in the immutable source record                  |
| `metadata`         | JSONB       | First-seen full normalized context; economic projection is hash-covered           |
| `display_snapshot` | JSONB       | Refreshable `ReceiptDisplaySnapshotV1`; latest login, URL, and metadata           |
| `payload_hash`     | TEXT        | NOT NULL — SHA-256 of `ReceiptEconomicCoreV1`, including canonical event identity |
...
DB trigger rejects DELETE and any UPDATE outside `display_snapshot`. The adapter refreshes the snapshot only after proving the incoming `ReceiptEconomicCoreV1` equals the stored core; a divergent core raises `RECEIPT_CONTENT_CONFLICT` and rolls back.
...
6. **Collect per source/sourceRef/stream** — Delegated to `CollectSourcesWorkflow` (child workflow). For each `activitySources` entry, for each `sourceRef`, for each `stream`:
   ...
   - Activity: insert `ingestion_receipts` with equal-core display refresh or divergent-core conflict (uses `adapter.version` as `producer_version`)
