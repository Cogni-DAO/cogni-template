...
- In Walk, remaining contributor supply is `budget_total - signed reservations`. This is auditable governance/process conservation, not an adversarial on-chain cap: Safe signers must verify every release against the policy, and DAO mint authority can still exceed it. Run's emissions controller is the first contract-enforced remaining-supply authority.
