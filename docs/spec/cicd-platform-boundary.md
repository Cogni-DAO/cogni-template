...
| Health / readiness                                          | k8s probes in base + `verify-buildsha.sh` `/version.buildSha` contract (Axiom 19); external workloads use the dedicated `ComputeWorkload` status controller | `/readyz`-as-rollout-proof or a request-lifecycle/background-timer reconciler                                                                                                                              |
...
| **P2 — SHIPPED (task.5044, PR #2077)** | `ComputeResourcePort` write half + `AkashComputeAdapter` (Console managed-wallet API; SDL rendered internally by `akash-sdl.ts`) live in prod behind `POST /api/v1/compute/deployments`; per ci-cd.md Axiom 23 (`AKASH_IS_NODE_APP_TARGET`, gated by story.5016) Akash is the standard node-app target, Cherry the state substrate | story.5016: provider mandate, egress catalog, DNS reconcile, server-side env, workload health reconcile |
