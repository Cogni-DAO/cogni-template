...
- **Entry Points:**
  ...
  - `@/features/*` → must use `services/` or `components/` (blocks `mappers/utils/constants/` from outside)
...
- **Features Cross-Privacy:** Features can import other features' internals (needs architectural decision: strict isolation vs. shared utilities pattern)
- **Contracts Entry Points:** No entry point rules (needs decision: public-by-default vs. index.ts pattern)
- **Test Boundaries:** Tests can import anything (needs decision: strict vs. pragmatic testing)
