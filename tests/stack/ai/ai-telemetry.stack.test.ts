...
describe("AI Telemetry Stack Tests", () => {
  ...
  const mockSessionUser: SessionUser = {
    ...
    walletAddress: "0xTelemetryTestUser1234567890123456789012",
  };

  const billingAccountId = "telemetry-stack-test-account";
  ...
  beforeEach(async () => {
    ...
    await db.insert(users).values({
      ...
      name: "Telemetry Stack Test User",
  ...
  describe("success path (P0 invariant)", () => {
    it("writes ai_invocation_summaries row on successful completion", async () => {
      ...
      // Query ai_invocation_summaries for rows matching this request
      // Note: We use requestId from context (ctx.reqId) which is generated by wrapRouteHandlerWithLogging
      // The response.requestId is different (generated by completion service)
      // But both should result in a row being written
      const rows = await db
        .select()
        .from(aiInvocationSummaries)
        .where(eq(aiInvocationSummaries.status, "success"));

      // Find the most recent row (our test's row)
      ...
      // 2. request_id is present (non-empty)
  ...
  describe("error path (P0 invariant)", () => {
    it("writes ai_invocation_summaries row on LLM error", async () => {
      // For this test, we need to force an LLM error deterministically.
      // We'll use an invalid model ID that LiteLLM will reject.
      // This avoids mocking and tests the real error path.
      ...
      // 4. trace_id is non-zero (OTel SDK running)
      expect(row.traceId).toMatch(/^[a-f0-9]{32}$/);
      expect(row.traceId).not.toBe(ZERO_TRACE_ID);
      ...
      // 7. latency_ms recorded (even on error)
