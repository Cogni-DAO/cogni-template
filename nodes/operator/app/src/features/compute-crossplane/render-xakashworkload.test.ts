...
const decl = (over: Partial<NodeWorkloadDeclaration> = {}): NodeWorkloadDeclaration => ({
  nodeId: NODE_ID,
...
describe("renderXAkashWorkload", () => {
  ...
  it("honors an explicit CR name override and providerConfigName", () => {
    const cr = renderXAkashWorkload(decl({ name: "demo-node-app", providerConfigName: "akash-prod" }));
  ...
  it("renders multiple services in order", () => {
    const cr = renderXAkashWorkload(
      decl({
        services: [
          { name: "app", image: "img/a:1", cpuUnits: 0.5, memoryMi: 256, storageMi: 512, port: 3000, visibility: "public" },
          { name: "worker", image: "img/b:1", cpuUnits: 0.25, memoryMi: 128, storageMi: 256, port: 4000, visibility: "private" },
        ],
      })
    );
  ...
  it("rejects a missing nodeId", () => {
    expect(() => renderXAkashWorkload(decl({ nodeId: "" }))).toThrow(/nodeId is required/);
  });

  it("rejects a missing publicHost", () => {
    expect(() => renderXAkashWorkload(decl({ publicHost: "" }))).toThrow(/publicHost is required/);
  });

  it("rejects an empty services list", () => {
    expect(() => renderXAkashWorkload(decl({ services: [] }))).toThrow(/at least one service/);
  });

  it("rejects a non-DNS-1123 CR name", () => {
    expect(() => renderXAkashWorkload(decl({ name: "Bad_Name" }))).toThrow(/DNS-1123/);
  });
    ...
    expect(() =>
      renderXAkashWorkload(
        decl({
          services: [
            { name: "app", image: "img/a:1", cpuUnits: 0.5, memoryMi: 256, storageMi: 512, port: 3000, visibility: "public" },
            { name: "app", image: "img/b:1", cpuUnits: 0.5, memoryMi: 256, storageMi: 512, port: 3001, visibility: "private" },
          ],
        })
      )
  ...
  it("rejects a non-positive resource field", () => {
    expect(() =>
      renderXAkashWorkload(
        decl({
          services: [
            { name: "app", image: "img/a:1", cpuUnits: 0, memoryMi: 256, storageMi: 512, port: 3000, visibility: "public" },
          ],
        })
      )
      ...
      renderXAkashWorkload(
        decl({
          services: [
            ...
            { name: "app", image: "img/a:1", cpuUnits: 0.5, memoryMi: 256, storageMi: 512, port: 3000, visibility: "world" },
          ],
        })
      )
