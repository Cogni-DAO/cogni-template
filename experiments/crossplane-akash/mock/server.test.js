// Unit tests for the mock operator compute API.
// Run: node --test   (Node >= 18, uses the built-in test runner)
//
// These prove the contract properties the Crossplane spike relies on — most
// importantly the no-double-spend idempotency that makes provider-http's
// re-issued CREATE crash-safe.
...
test("POST provisions and returns a ProvisionOutput with a leaseId", () => {
  ...
  assert.match(r.body.leaseId, /^\d+$/); // dseq-like handle
...
test("no-double-spend: repeated POST for the same nodeId returns the SAME leaseId", () => {
  ...
  // Three provision attempts, exactly ONE lease minted — the crash-safety witness.
...
test("PUT updates in place and keeps the same leaseId; specHash changes on drift", () => {
  ...
  assert.equal(updated.body.leaseId, created.body.leaseId); // same lease
  assert.notEqual(updated.body.specHash, before); // drift observed
...
test("DELETE releases the lease; subsequent GET is 404; balance recovers", () => {
  ...
  assert.equal(before.body.balances[0].remaining, 99.5); // one active lease escrows 0.5
  ...
  assert.equal(bal.body.balances[0].remaining, 100); // escrow returned
});
