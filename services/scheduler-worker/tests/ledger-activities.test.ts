...
describe("finalizeEpoch — per-node distribution config (bug.5020)", () => {
  ...
  function makeConfigClient(
    impl: DistributionConfigHttpClient["resolveForNode"]
  ): DistributionConfigHttpClient {
    return { resolveForNode: vi.fn(impl) };
  }

  function activitiesWith(
    ...
    vi.mocked(verifyTypedData).mockResolvedValue(true);
    return createAttributionActivities({
      attributionStore: store,
      sourceRegistrations: new Map(),
      registries,
      nodeId: NODE_ID,
      ...
      walletResolver: (opts.withWalletResolver === false
        ...
        typeof createAttributionActivities
      >[0]["walletResolver"],
  ...
  async function runFinalize(activities: ReturnType<typeof activitiesWith>) {
    return activities.finalizeEpoch({
      epochId: "7",
      signature: "0xdeadbeef",
      signerAddress: SIGNER,
    });
