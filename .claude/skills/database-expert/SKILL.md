...
> **⚠️ The `db_backup.completed` log does NOT mean the app dump succeeded (prod, 2026-08-05).** `backup.sh` connects to `postgres:5432` over TCP as user `postgres`, but on prod that role's TCP password auth **fails** (`FATAL: password authentication failed for user "postgres"`) — yet the script still emits `event="db_backup.completed" cluster="app"` and writes a **0-byte `globals.sql`** with no per-DB `pg_dump`. So the completion log + a Loki hit are NOT proof of a real backup; **verify the dump file is non-empty**, not just that the event fired (a silent-success anti-pattern). Until fixed, a trustworthy manual backup is `docker exec cogni-runtime-postgres-1 pg_dump -U postgres -Fc cogni_operator` (local **socket** → peer/trust auth, no TCP password) piped to an off-host encrypted file. Root fix: give `backup.sh` a working DB credential (the app already reaches Postgres via its own DSN role — reuse that, not bare `postgres`) and make the script fail-hard (non-zero exit + no `completed` event) when `pg_dump` errors.
